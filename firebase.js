...
/* ===================== LIVE DASHBOARD DISPLAY ENGINE ===================== */
  let visitorChartInstance = null;
  let deviceChartInstance = null;

  function setupLiveAnalyticsDashboard() {
    const totalCountEl = document.getElementById("visitor-count");
    const todayCountEl = document.getElementById("today-visitors");
    const activeUsersEl = document.getElementById("activeUsers");
    const lastUpdatedEl = document.getElementById("lastUpdated");

    if (!totalCountEl || !db) return;
    ...
    onSnapshot(collection(db, "visitors"), (snapshot) => {
      ...
      const todayHits = docs.filter(v => {
        if (!v.lastSeen) return false;
        return v.lastSeen.toDate() >= startOfToday;
      }).length;
      todayCountEl.textContent = todayHits;

      const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000);
      const liveNow = docs.filter(v => {
        if (!v.lastSeen) return false;
        return v.lastSeen.toDate() >= fiveMinutesAgo;
      }).length;
      activeUsersEl.textContent = liveNow || 1;

      const now = new Date();
      lastUpdatedEl.textContent = `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}`;
      
      // Pass data array downstream to redraw UI graphs and list progress cards
      processTrafficMetrics(docs);
      renderLiveCharts(docs);
    });
  }

  function processTrafficMetrics(visitorData) {
    let referrers = { Direct: 0, GitHub: 0, LinkedIn: 0, Search: 0 };

    visitorData.forEach(v => {
      const ref = (v.referrer || "").toLowerCase();
      if (ref.includes("github")) referrers.GitHub++;
      else if (ref.includes("linkedin")) referrers.LinkedIn++;
      else if (ref.includes("google") || ref.includes("bing")) referrers.Search++;
      else referrers.Direct++;
    });

    updateProgressUI("Direct", referrers.Direct, visitorData.length);
    updateProgressUI("GitHub", referrers.GitHub, visitorData.length);
    updateProgressUI("LinkedIn", referrers.LinkedIn, visitorData.length);
    updateProgressUI("Search", referrers.Search, visitorData.length);
  }

  function updateProgressUI(label, count, total) {
  ...
  function renderLiveCharts(visitorData) {
    // 1. Compile Traffic Over Time metrics (Last 7 Days)
    const dailyCounts = {};
    for (let i = 6; i >= 0; i--) {
      const d = new Date();
    ...
    // 2. Compile Device splits metrics
    let mobileCount = 0;
    let desktopCount = 0;
    ...
    visitorData.forEach(v => {
      ...
      // Tally days
      if (v.lastSeen) {
        const dateKey = v.lastSeen.toDate().toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
    ...
    const chartDataValues = Object.values(dailyCounts);

    // Render Traffic Overview Area Chart
    const ctxVisitor = document.getElementById('visitorChart');
    if (ctxVisitor) {
      if (visitorChartInstance) visitorChartInstance.destroy(); // Clear stale reference canvas
    ...
    if (ctxDevice) {
      if (deviceChartInstance) deviceChartInstance.destroy(); // Clear stale reference canvas
